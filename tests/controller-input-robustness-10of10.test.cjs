...
test('independent controller sources cannot release a key still held by another source', () => {
  assert.match(inputState, /val keyHoldCounts: IntArray = IntArray\(10\)/);
  assert.match(inputState, /var generation: Long = 0L/);
  assert.match(multiplayer, /internal fun MainActivity\.setGameplayKeyHeld\(key: Int, pressed: Boolean\)/);
  assert.match(multiplayer, /val previousCount = gameplayInputState\.keyHoldCounts\[key\]/);
  assert.match(multiplayer, /if \(previousCount == 0 \|\| nextCount == 0\)[\s\S]*setGameplayKey\(key, nextCount > 0\)/);
  assert.match(gameplay, /gameplayInputState\.generation\+\+/);
  assert.match(gameplay, /gameplayInputState\.keyHoldCounts\.fill\(0\)/);
});

test('D-pad has stale-stream recovery and releases on CANCEL/OUTSIDE', () => {
  assert.match(layout, /gameplayInputState\.activeDpadPointerId != MotionEvent\.INVALID_POINTER_ID \|\| gameplayInputState\.activeDpadMask != 0/);
  assert.match(layout, /MotionEvent\.ACTION_CANCEL,[\s\S]*MotionEvent\.ACTION_OUTSIDE[\s\S]*finishDpadGesture\(view\)/);
  assert.match(layout, /event\.findPointerIndex\(gameplayInputState\.activeDpadPointerId\)/);
});
...
test('single keys use source-safe hold transitions and recover stale ownership', () => {
  ...
  assert.match(layout, /gestureGeneration = gameplayInputState\.generation/g);
  assert.match(layout, /gestureGeneration != gameplayInputState\.generation/g);
...
test('combo controls own one pointer, retain thumb drift, and never steal another pointer', () => {
  ...
  assert.match(multi, /gestureGeneration = gameplayInputState\.generation/g);
  assert.match(multi, /gestureGeneration != gameplayInputState\.generation/g);
...
test('Turbo AB is pointer-owned and balances every generated A/B hold', () => {
  ...
  assert.match(turbo, /gestureGeneration = gameplayInputState\.generation/g);
  assert.match(turbo, /gestureGeneration != gameplayInputState\.generation/g);
